...
class TrainAndTestMIN(object):
    def __init__(self, cfg):
        ...
        self.min_instance = MIN(n_states, n_actions, cfg)
    ...
    def train(self):
        ...
        for i_ep in range(self.cfg.train_eps):
            ...
            while True:
                ep_step += 1
                # 与环境互动
                action = self.min_instance.choose_action(state)  # 选择动作
                if action == 0:
        ...
        self.env.close()
        return rewards, ma_rewards, time_delays, ma_finished_rate, rsu1_cap, rsu2_cap, rsu3_cap, costs, bat_lifes, vehicle_offloading_rate, rsu1_offloading_rate, rsu2_offloading_rate, rsu3_offloading_rate, rsu_total_cap
